...
The seven tetromino colours are chosen from this palette rather than from the
standard Tetris colours, so the board reads as one piece of art. They are also
deliberately spread around the hue circle so a CNN can separate them from a
handful of pixels — ``test_legibility`` asserts that property rather than
trusting it.
"""
