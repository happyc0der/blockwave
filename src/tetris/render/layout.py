...
``cell_px``. The human window and the agent's observation therefore run through
identical layout code and differ only in how many pixels a cell is worth —
which is what lets both look at the same picture without the training loop
paying for pixels it is about to throw away.
"""
...
#: appears — necessary for a human, and doubly so for an agent that has only
#: the picture to go on.
SPAWN_ROWS_SHOWN = 2
...
#: Small cells for agent observations. Scene works out at 154x162, which
#: downsamples cleanly to the 84x84 a CNN expects.
AGENT_CELL_PX = 7
