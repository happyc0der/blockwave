...
touches an audio device, which is what lets the same code drive a 60 fps arcade
cabinet and run headless at hundreds of thousands of steps per second.

The engine advances by ``step(action, dt)``. Time is passed in explicitly rather
than read from a clock, so a fixed-timestep game loop, a replay and a headless
script all drive it identically.
"""
...
class EngineConfig:
    ...
    ``start_level`` and ``gravity_scale`` set where on the difficulty curve a
    game begins and how fast gravity runs relative to it.
    """
...
@dataclass(slots=True)
class Stats:
    """Cumulative counters for one game."""
...
class Engine:
    ...
    def telemetry(self) -> dict[str, float | int | bool]:
        """A snapshot of the game as plain data.

        Everything a HUD, a logger or an external driver could want, computed
        once and handed over in one dict.
        """
        board = self.board
        stats = self.stats
        return {
