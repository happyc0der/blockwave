...
Keeping these out of pygame means a frame is composited entirely in numpy and
only becomes a surface at the moment it is put on screen.
"""
...
def shake_offset(frame: np.ndarray, dx: int, dy: int) -> np.ndarray:
    ...
    Moving the board rather than covering it keeps every cell readable.
    """
    if dx == 0 and dy == 0:
        return frame
