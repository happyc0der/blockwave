...
This is the only place the game is drawn. Every frame anyone sees comes out of
:meth:`Compositor.render`, at whatever ``cell_px`` and :class:`VisualProfile` is
asked for.
...
class Compositor:
    ...
    def render(
        ...
        *,
        ...
        frame = self._post_process(frame, shake)
        frame = np.clip(frame, 0.0, 255.0).astype(np.uint8)
        return np.ascontiguousarray(frame)

    def _draw_cells(self, frame: np.ndarray, engine: Engine) -> None:
    ...
    def _draw_clear_wipe(self, frame: np.ndarray, engine: Engine) -> None:
    ...
    def _blit_cells(
        ...
        *,
        ...
        ``np.repeat`` twice beats a per-cell loop by a wide margin, and the
        cell paint is the only part of a frame that is not cached.
        """
        mask = grid != 0
        if not mask.any():
            return
    ...
    def _draw_hud(self, frame: np.ndarray, engine: Engine) -> None:
        layout = self.layout
        cell = layout.cell_px
        scale = max(1, cell // 7)
    ...
    def _post_process(self, frame: np.ndarray, shake: tuple[int, int]) -> np.ndarray:
        """Global passes over the finished frame.

        Each is gated on the profile, and each modulates the whole frame
        uniformly — none of them can single out or obscure a playfield cell.
        """
        profile = self.profile

        # Scaled by the profile, so a profile that declares no shake genuinely
        # has none.
        frame = effects.shake_offset(
            frame,
            int(round(shake[0] * profile.shake)),
            int(round(shake[1] * profile.shake)),
        )

        if profile.bloom > 0.0:
            radius = max(1, self.layout.cell_px // 3)
            frame = effects.bloom(frame, profile.bloom, radius)
        if profile.chromatic > 0.0:
            frame = effects.chromatic(frame, profile.chromatic)
        if profile.scanlines > 0.0:
            frame = effects.scanlines(frame, profile.scanlines)
