...
from ..core.engine import EngineConfig, Engine
...
class ShakeState:
    ...
    not scene content: the compositor draws a board, not a reaction to one.
    """
...
class Game:
    def __init__(
        ...
        *,
        ...
        self.engine = Engine(EngineConfig(seed=seed, start_level=start_level))
        self.layout = Layout(cell_px=cell_px)
        self.compositor = Compositor(self.layout, profile)
        self.display = Display(self.compositor.frame_size[::-1], title="BLOCKWAVE")
    ...
    def _panel_lines(self) -> list[PanelLine]:
        ...
        if self.scene is Scene.TITLE:
            lines = [
                PanelLine("BLOCKWAVE", "hero", TEXT_HOT),
                PanelLine("MIAMI NEON", "body", PURPLE),
            ]
            if self.scores.best:
